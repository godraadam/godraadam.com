...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  // node_modules/@shaun/alpinejs-router/dist/module.esm.js
  function src_default2(Alpine2) {
    ...
    const router = {
      ...
      config(config = {}) {
        if (config.mode !== "hash" && config.base && config.base.endsWith("/"))
    ...
    Alpine2.magic("router", () => router);
    Alpine2.effect(() => {
      state.query = state.href.indexOf("?") > -1 ? Object.fromEntries(new URLSearchParams(state.href.split("?").pop()).entries()) : {};
    });
    ...
    function is({ paths, parseParams = false }) {
      ...
      const [pathname] = state.mode === "hash" ? url.hash.slice(1).split("?") : [url.pathname.replace(state.base, "")];
    ...
    Alpine2.directive("route", (el, { modifiers, expression }, { effect: effect3, cleanup: cleanup2 }) => {
      ...
      function show() {
        ...
        const make = () => {
          ...
          Alpine2.addScopeToNode(clone2, {}, el);
          Alpine2.mutateDom(() => {
            el.after(clone2);
            Alpine2.initTree(clone2);
          });
          el._x_currentIfEl = clone2;
          el._x_undoIf = () => {
            clone2.remove();
          ...
          Alpine2.nextTick(() => inMakeProgress.delete(expression));
        };
    ...
    Alpine2.directive("link", (el, { modifiers, expression }, { evaluate: evaluate2, effect: effect3, cleanup: cleanup2 }) => {
      ...
      if (modifiers.includes("activity")) {
        const classes = expression ? evaluate2(expression) : {};
        classes.active ??= "active";
        classes.exactActive ??= "exact-active";
        effect3(() => {
          const [elUrl, stateUrl] = [new URL(el.href), new URL(state.href)];
          const [l, r] = state.mode === "hash" ? [elUrl.hash.slice(1).split("?").shift(), stateUrl.hash.slice(1).split("?").shift()] : [elUrl.pathname, stateUrl.pathname];
